...
# ── параметры установки ──────────────────────────────────────────────────
DOMAIN=""            # почтовый домен, например example.com
FQDN=""              # имя почтового сервера, например mail.example.com
...
# В VNC-консоли сообщения ядра сыплются прямо поверх диалогов и рвут рамки.
# Приглушаем их на время работы и возвращаем как было при выходе.
KMSG_LEVEL=$(cut -d$'\t' -f1 /proc/sys/kernel/printk 2>/dev/null || echo 4)
quiet_console() { dmesg -n 1 2>/dev/null || true; }
restore_console() { dmesg -n "${KMSG_LEVEL:-4}" 2>/dev/null || true; }
...
Пример: example.com
Адреса получатся вида info@example.com" "example.com" \
valid_domain "Домен пишется без http:// и без косой черты, латиницей,
и содержит хотя бы одну точку. Пример: example.com")
...
valid_ip "Нужен адрес вида 203.0.113.10 — четыре числа
...
Пример: info, post, sales" "info, post" \
valid_boxes "Имена пишутся латиницей, через запятую, без @ и без
пробелов внутри имени. Пример: info,post,sales")
...
step_firewall() {
  ...
  # По умолчанию ufw пишет каждую заблокированную попытку в консоль.
  # На сервере с внешним IP это сотни строк в минуту — выключаем.
  run "ufw logging off"
}
step_docker() {
...
step_boxes() {
  ...
  local b i
  # Ждём, пока контейнер начнёт принимать команды. Долго ждать нельзя:
  # без ящиков сервер выключается через 120 секунд после старта.
...
run_step() {
  ...
  while :; do
    ...
    local detail
    detail=$(tail -n 5 "$LOG" 2>/dev/null | cut -c1-70 | sed 's/^/  /')
    # Если дело в почтовом сервере — полезнее его собственный журнал
    if docker ps -a --format '{{.Names}}' 2>/dev/null | grep -qx mailserver; then
      case "$fn" in
        step_up|step_wait|step_boxes|step_dkim)
          detail=$(docker logs --tail 6 mailserver 2>&1 | cut -c1-70 | sed 's/^/  /') ;;
      esac
    fi
...
# Готовность проверяем снаружи, с хоста: порт 25 опубликован наружу,
# и это тот же путь, которым придёт чужой почтовый сервер. Проверка
# внутри контейнера через ss ненадёжна — утилиты может не быть,
# а docker exec не сработает, пока контейнер перезапускается.
v_wait() {
  timeout 5 bash -c 'exec 3<>/dev/tcp/127.0.0.1/25' 2>/dev/null && return 0
...
f_wait() {
  # Причина первая: не создан ни один ящик. docker-mailserver не запускает
  # Dovecot без ящиков и выключается через 120 секунд, уходя в перезапуск.
  if docker logs --tail 40 mailserver 2>&1 | grep -q 'at least one mail account'; then
    log "сервер не стартует без ящиков — создаю их"
    run "cd $DIR && docker compose up -d"
    step_boxes
    sleep 15
    return 0
  ...
  # Причина вторая, реже:
  # SSL_TYPE=letsencrypt при отсутствующем сертификате: контейнер
  # уходит в бесконечный перезапуск.
  if grep -q 'SSL_TYPE=letsencrypt' "$DIR/compose.yaml" 2>/dev/null && \
     [ ! -s "$DIR/docker-data/certbot/certs/live/$FQDN/fullchain.pem" ]; then
    log "сертификата нет, а SSL_TYPE=letsencrypt — перевожу на самоподписанный"
    run "sed -i 's|- SSL_TYPE=letsencrypt|- SSL_TYPE=self-signed|' $DIR/compose.yaml"
    CERT_FALLBACK=1
    run "cd $DIR && docker compose up -d --force-recreate"
    sleep 15
    return 0
  fi
  # Иначе — обычный перезапуск с пересозданием контейнера
  run "cd $DIR && docker compose down"
  run "cd $DIR && docker compose up -d --force-recreate"
  sleep 15
}
...
STEPS=(
  ...
  "step_up|Запуск сервера|86|v_up|f_up"
  "step_boxes|Ящики и правила пересылки|92|v_boxes|f_boxes"
  "step_wait|Ожидание готовности|96|v_wait|f_wait"
...
Строка вида 10-113-0-203.static.hoster.ru — стоит
